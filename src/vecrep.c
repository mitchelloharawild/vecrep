...
#define VREP_TIMES(x)                                                          \
  ((R_xlen_t)INTEGER_ELT(VECTOR_ELT(R_altrep_data1(x), 1), 0))
...
SEXP make_rep_real(SEXP parent, SEXP times) {
  ...
  */
  ...
  /* there was a bug in R_MakeExternalPtr which didn't increment ref counts of
     Prot, but setter did. Fixed already in R-devel and R-patched but
     initializing to R_NilValue then setting it works backwards-compatibly. */
...
static R_xlen_t vrep_real_Get_region(SEXP x, R_xlen_t i, R_xlen_t n,
  ...
  SEXP exp = VREP_EXPANDED(x);
  if (exp != R_NilValue)
    return REAL_GET_REGION(exp, i, n, buf);
...
static const R_CallMethodDef CallEntries[] = {
    {"make_rep_real", (DL_FUNC)&make_rep_real, -1}, {NULL, NULL, 0}};
